...
public class MenuListener implements ActionListener, ItemListener{
  ...
  private void run() throws IOException {
    ...
    if ((exe.exists()) && (exe.canExecute()))
      Runtime.getRuntime().exec(exe.getAbsolutePath(), null, new File(path));
    else
      JOptionPane.showMessageDialog(this.fileTree, "Currently only supported in Windows.");
  }
  ...
  protected void setPaths()
  {
    ...
    setWesPath.addActionListener(new ActionListener()
    {
      public void actionPerformed(ActionEvent e) {
        ...
        if (returnVal == 0)
          wesPathField.setText(chooser.getSelectedFile().getAbsolutePath());
      }
    });
